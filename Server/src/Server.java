            ...
            try {
                ...
                while (true) {
                    ...
                    if (inputABytes.size() == 9){
                        ...
                        for (int i = 8; i >= 0; i--){
                            //wypycha najmniejznaczacy bit
                            byte tmpByte = (byte) (inputABytes.get(i).byteValue() >>> 1);
                            //if least significant bit of i-1 == 1
                            if (i > 0 && ((inputABytes.get(i-1).byteValue() & 1) == 1)){
                                tmpByte += 0b10000000;
                            }
                            inputABytes.get(i).valueOf(tmpByte);
                        }
                        //TODO: read Data length into long (it's inputABytes[from 1 to 8]) and read data and sessionID
